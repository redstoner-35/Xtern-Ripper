...
+ `PCB-DiffTorch-PowerStage`：1.6mm 4层板，默认层叠顺序（Top GND VCC Bottom）必须过孔塞树脂+沉金。内外层建议加厚为2Oz，如果不加厚则最大只能输出20A电流。
+ `PCB-DiffTorch-MCU`:1.6mm 4层板，默认层叠顺序（Top GND VCC Bottom）必须过孔塞树脂，沉金可选，外层1Oz，内层0.5Oz即可。
+ `PCB-DiffTorch-MoonLight`:1.0mm 4层板，默认层叠顺序（Top GND VCC Bottom）必须过孔塞树脂，沉金可选，外层1Oz，内层0.5Oz即可。
+ `PCB-DiffTorch-BattTypecCharger-V2`:1.6mm 4层板，默认层叠顺序（Top GND VCC Bottom）必须过孔塞树脂+沉金，外层1Oz，内层0.5Oz即可。
+ `PCB-DiffTorch-BattHolder`:1.6mm厚度，其中[PCB-Bottom.PcbDoc](/PCB-DiffTorch-BattHolder/Bottom/PCB-Bottom.PcbDoc)为双层板，1Oz铜厚（有条件可以打2Oz内阻更低）喷锡即可。[顶层均衡板](/PCB-DiffTorch-BattHolder/PCB1.PcbDoc)则为4层板，默认层叠顺序（Top GND VCC Bottom）必须过孔塞树脂，沉金可选，外层1Oz，内层0.5Oz即可，有条件可以内外层加厚为2Oz。
+ `PCB-DiffTorch-SideKey`:1.0mm厚度双面板，铜厚1Oz喷锡即可。

### 焊接调试注意事项

这个工程包含150+元器件，五块PCB，而且有非常小且高引脚密度的QFN和SON-8芯片（最小封装2x2mm），大量的SOT323和0402原件，因此对焊接者的手工和耐心有非常高的要求。焊接和调试工具方面需要准备的东西如下：

+ 加热台 **注意!那种直接一个插头怼市电的不行，必须有恒温功能且温控器的温度调节功能无过冲，否则工程中不耐热的芯片会损坏！**
...
+ 万用表(至少四位半，有条件的话建议准备6位半台表)
+ 可调电源(具备恒压恒流功能，电压和输出电流可调且最好带输出电压电流显示，至少能提供12V5A)
+ 示波器(带宽100M及以上，数字和模拟示波器均可，需具备占空比和频率测量以及测量值趋势分析功能)
